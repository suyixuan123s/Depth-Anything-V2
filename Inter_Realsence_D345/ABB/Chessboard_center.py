...
#
#             # 提取标定板坐标系的 X、Y、Z 轴方向向量
#             x_axis = rotation_matrix[:, 0]  # X 轴方向
#             y_axis = rotation_matrix[:, 1]  # Y 轴方向
#             z_axis = rotation_matrix[:, 2]  # Z 轴方向
#
#             print(f"标定板坐标系 X 轴在相机坐标系中的方向: {x_axis}")
#             print(f"标定板坐标系 Y 轴在相机坐标系中的方向: {y_axis}")
#             print(f"标定板坐标系 Z 轴在相机坐标系中的方向: {z_axis}")
#
#             # 显示带有原点标注的图像
#             cv2.imshow("Chessboard with Origin", image)
#             cv2.waitKey(0)
